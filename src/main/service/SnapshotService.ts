...
import { AssetSnapshotForm, ReqSearchModel } from '../../common/ReqModel';
import { SnapshotEntity } from '../entity/Entity';
import { ResAssetGroupModel, ResSnapshotModel, ResStockEvaluateModel, ResTradeModel } from '../../common/ResModel';
...
export default class SnapshotService {
  ...
  static async saveSnapshot(snapshot: AssetSnapshotForm) {
    // 1. 스냅샷 저장
   ...
   * 스냅샷 저장
   */
  private static async saveSnapshotEntity(snapshot: AssetSnapshotForm) {
   ...
   * 계좌 평가 금액 저장
   */
  private static async saveAssetGroups(snapshot: AssetSnapshotForm, snapshotEntity: SnapshotEntity) {
      ...
      .map(async (accountGroupList, accountType) => {
        ...
        const balancePromises = accountGroupList.map(async (account) => {
          ...
          return {
            ...
            totalBuyAmountKrw,
            totalEvaluateAmountKrw: totalEvaluateAmountKrw,
          };
        });
   ...
   * 환율 저장
   */
  private static async saveExchangeRates(snapshot: AssetSnapshotForm, snapshotEntity: SnapshotEntity) {
